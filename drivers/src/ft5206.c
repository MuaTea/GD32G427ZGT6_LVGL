...
uint8_t ft5206_init(void)
{
    ...
    gpio_mode_set(GPIOB, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP, GPIO_PIN_1);  /* FT_INT引脚模式设置,上拉输入 */
...
uint8_t ft5206_scan(uint8_t mode)
{
    ...
    if ((t % 10) == 0 || t < 10)   /* 空闲时,每进入10次CTP_Scan函数才检测1次,从而节省CPU使用率 */
	  {			
        ft5206_rd_reg(FT_REG_NUM_FINGER, &sta, 1);  /* 读取触摸点的状态 */
					 
        if ((sta & 0XF) && ((sta & 0XF) < 6))
				{
            temp = 0XFF << (sta & 0XF);           /* 将点的个数转换为1的位数,匹配tp_dev.sta定义 */
