 ...
 *              1,进行过校准
 */
uint8_t rtp_init(void)
{
    rcu_periph_clock_enable(RCU_GPIOB);     /* 使能GPIOB时钟 */
    rcu_periph_clock_enable(RCU_GPIOC);     /* 使能GPIOC时钟 */
    rcu_periph_clock_enable(RCU_GPIOF);     /* 使能GPIOF时钟 */
  
    gpio_mode_set(GPIOB, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP, GPIO_PIN_1);  /* T_PEN引脚模式设置,上拉输入 */
    gpio_mode_set(GPIOB, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP, GPIO_PIN_2);  /* T_MISO引脚模式设置,上拉输入 */
